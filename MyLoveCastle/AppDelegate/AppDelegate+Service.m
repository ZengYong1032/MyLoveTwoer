...
-(NSMutableArray *)addLogsDataWithDataType:(ZYLogDataType)datatype
{
    ...
    NSArray *logTagNames = @[@"0",@"0",@"0",@"6",@"6",@"0"];
    NSArray *categoryIds= @[@0,@1,@2,@3,@0,@1,@4];
    NSArray *logIds = @[@0,@1,@2,@3,@4,@5,@6];
    ...
    for (int i=0; i<[loges count]; i++)
    {
        ...
        li.contents = loges[i];
        li.typeTag = logTagNames[i];
        li.logId = [logIds[i] integerValue];
        li.categoryId = [categoryIds[i] integerValue];
    ...
    {
        case 0:
            resultDatas = [NSMutableArray arrayWithObjects:logs[0],logs[1],logs[2],logs[5], nil] ;
            break;
            
        case 1:
            resultDatas =[NSMutableArray arrayWithObjects:logs[3],logs[4], nil] ;
            break;
            
        case 2:
            resultDatas = [NSMutableArray arrayWithArray:logs];
            break;
...
-(NSMutableArray *)setupAnniversaryDataArray
{
    ...
    NSArray *dataA = @[@{@"anniName":@"Fall In love",@"dateStr":@"2015-04-24",@"slogan":@"Loved Two Anniversaries !",@"bgImgName":@"love.png",@"planContent":@"Have no plan yet.",@"annCount":@"2 anniversaries."},
                       
                       @{@"anniName":@"Marriage",@"dateStr":@"2016-10-20",@"slogan":@"One-Year Anniversaries Of Marriage",@"bgImgName":@"marriage.png",@"planContent":@"Yes,haved a plan .",@"annCount":@"1 anniversaries."},
                       
                       @{@"anniName":@"Hand In Hand",@"dateStr":@"2014-09-24",@"slogan":@"Third Anniversaries Of Hands",@"bgImgName":@"hands.png",@"planContent":@"Have no plan yet.",@"annCount":@"3 anniversaries."}];
    NSArray *anniIds = @[@0,@1,@2];
    int i=0;
    for (NSDictionary *dic in dataA)
    {
        ZYAnniversary_Model *am = [ZYAnniversary_Model new];
        am.anniName = dic[@"anniName"];
        am.slogan = dic[@"slogan"];
        am.dateStr = dic[@"dateStr"];
        am.bgImgName = dic[@"bgImgName"];
        am.planContent = [ZYConvertTool sharedDecriptionAttributedString:dic[@"planContent"]];
        am.annCount = dic[@"annCount"];
        am.descriptionContent = [ZYConvertTool sharedDecriptionAttributedString:@"There is no description of anniversary."];
        am.anniID = [anniIds[i] integerValue];
        
        [scrollDataArray addObject:am];
        i++;
    }
